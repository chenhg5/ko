...
func init() {
	...
	var config = (*ucenter.GetConfig())

	var err error
	SqlDB, err = sql.Open("mysql", config["database_ip"]+
		":"+ config["database_pwd"]+ "@tcp("+ config["database_ip"]+
		":"+ config["database_port"]+ ")/"+ config["database_name"]+ "?charset=utf8mb4")
